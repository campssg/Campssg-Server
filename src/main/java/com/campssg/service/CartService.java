...
public class CartService {
    ...
    // 장바구니 추가(로그인하자마자 메인화면에서 장바구니 추가)
    public void addCart() {
        User user = SecurityUtil.getCurrentUsername().flatMap(userRepository::findByUserEmail).orElseThrow(); // 현재 로그인하고 있는 사용자 정보 가져오기
        cartRepository.save(Cart.builder().user(user).totalCount(0).totalPrice(0).build());
    }

    // 내 장바구니 정보 확인 - 담겨 있는 상품 목록, 총 개수, 총 가격
     public CartInfoResponseDto getCartInfo() {
         User user = SecurityUtil.getCurrentUsername().flatMap(userRepository::findByUserEmail).orElseThrow(); // 현재 로그인하고 있는 사용자 정보 가져오기
         Cart cart = cartRepository.findByUser_userId(user.getUserId()).orElseThrow(); // 사용자 아이디 값으로 장바구니 가져오기
